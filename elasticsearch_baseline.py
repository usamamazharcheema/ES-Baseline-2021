...
def create_connection(conn_string='http://localhost:9200'):
...
def format_scores(scores):
    ...
    for iclaim_id in scores:
        for i, (vclaim_id, score) in enumerate(scores[iclaim_id]):
            output_string += f"{iclaim_id}\tQ0\t{vclaim_id}\t{1}\t{score}\telasic\n"
...
def run_baselines(args):
    ...
    vclaims_df = pd.read_csv('verified_claims.docs.tsv', sep='\t', index_col=0)
    vclaims_df = vclaims_df.reset_index()
    vclaims = {}
    for index, row in vclaims_df.iterrows():
        dict_vclaim = {}
        dict_vclaim['vclaim_id'] = row['vclaim_id']
        dict_vclaim['vclaim'] = row['vclaim']
        dict_vclaim['title'] = row['title']
        vclaims[index] = dict_vclaim
    ...
    ngram_baseline_fpath = join(ROOT_DIR, f'checkthat2020_submission-master/baselines/data/subtask_{args.subtask}_elastic_baseline_{args.lang}_{basename(args.dev_file_path)}')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dev_file_path", "-d", required=True, type=str,
                        help="The absolute path to the dev data")
    parser.add_argument("--vclaims_dir_path", "-v", required=True, type=str,
                        help="The absolute path to the directory with the verified claim documents")
    parser.add_argument("--iclaims_file_path", "-i", required=True,
    ...
    parser.add_argument("--conn", "-c", default="http://localhost:9200",
